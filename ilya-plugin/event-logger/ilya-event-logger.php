...
class ilya_event_logger
{
		...
		if (ilya_opt('event_logger_to_database')) {
			...
			if (!in_array($tablename, $table_list)) {
				...
				return 'CREATE TABLE ^eventlog (' .
					...
					'handle VARCHAR(' . ILYA_DB_MAX_HANDLE_LENGTH . '),' .
	...
	public function admin_form(&$ilya_content)
	{
		...
		return array(
			...
			'fields' => array(
				array(
					'label' => 'Log events to <code>' . ILYA_MYSQL_TABLE_PREFIX . 'eventlog</code> database table',
			...
			if (is_resource($file)) {
				if (flock($file, LOCK_EX)) {
					if (!$exists && filesize($filename) === 0 && !ilya_opt('event_logger_hide_header')) {
						$string = "IlyaIdea " . ILYA_VERSION . " log file generated by Event Logger plugin.\n" .
