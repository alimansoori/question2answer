...
class ilya_html_theme_layer extends ilya_html_theme_base {
	...
	function doctype() {
		...
		$this->pluginurl = ilya_opt('site_url').'ilya-plugin/extra-question-field/';
		if($this->template == 'question') {
			if(isset($this->content['q_view']['raw']['postid']))
				$this->extradata = $this->ilya_eqf_get_extradata($this->content['q_view']['raw']['postid']);
		}
	}
	function head_script() {
		ilya_html_theme_base::head_script();
		if(count($this->extradata) && $this->ilya_eqf_file_exist() && ilya_opt(ilya_eqf::LIGHTBOX_EFFECT)) {
	...
	function main() {
		if($this->template == 'ask') {
			if(isset($this->content['form']['fields']))
				$this->ilya_eqf_add_field(null, $this->content['form']['fields'], $this->content['form']);
		} else if(isset($this->content['form_q_edit']['fields'])) {
				$this->ilya_eqf_add_field($this->content['q_view']['raw']['postid'], $this->content['form_q_edit']['fields'], $this->content['form_q_edit']);
		}
	...
	function ilya_eqf_add_field($postid, &$fields, &$form) {
		global $ilya_extra_question_fields;
		$multipart = false;
		for($key=ilya_eqf::FIELD_COUNT_MAX; $key>=1; $key--) {
			if((bool)ilya_opt(ilya_eqf::FIELD_ACTIVE.$key)) {
				$field = array();
				$name = ilya_eqf::FIELD_BASE_NAME.$key;
				$field['label'] = ilya_opt(ilya_eqf::FIELD_PROMPT.$key);
				$type = ilya_opt(ilya_eqf::FIELD_TYPE.$key);
				switch ($type) {
				case ilya_eqf::FIELD_TYPE_FILE:
					$field['type'] = 'custom';
					...
					if(!empty($value)) {
						...
						$original = $filename;
						$width = $this->ilya_eqf_get_image_width($blob['content']);
						if($width > ilya_opt(ilya_eqf::THUMB_SIZE))
							$width = ilya_opt(ilya_eqf::THUMB_SIZE);
						if($format == 'jpg' || $format == 'jpeg' || $format == 'png' || $format == 'gif')
							$original = '<IMG SRC="'.$imageurl.'&ilya_size='.$width.'" ALT="'.$filename.'" ID="'.$name.'-thumb" CLASS="'.ilya_eqf::FIELD_BASE_NAME.'-thumb"/>';
						$original = '<A HREF="'.$imageurl.'" TARGET="_blank" ID="'.$name.'-link" CLASS="'.ilya_eqf::FIELD_BASE_NAME.'-link">' . $original . '</A>';
						$original .= '<INPUT TYPE="checkbox" NAME="'.$name.'-remove" id="'.$name.'-remove" CLASS="'.ilya_eqf::FIELD_BASE_NAME.'-remove"/><label for="'.$name.'-remove">'.ilya_lang(ilya_eqf::PLUGIN.'/extra_field_remove').'</label><br>';
					...
					break;
				default:
					$field['type'] = ilya_opt(ilya_eqf::FIELD_TYPE.$key);
					$field['tags'] = 'NAME="'.$name.'"';
					$options = $this->ilya_eqf_options(ilya_opt(ilya_eqf::FIELD_OPTION.$key));
					if (ilya_opt(ilya_eqf::FIELD_ATTR.$key) != '')
						$field['tags'] .= ' '.ilya_opt(ilya_eqf::FIELD_ATTR.$key);
					if ($field['type'] != ilya_eqf::FIELD_TYPE_TEXT && $field['type'] != ilya_eqf::FIELD_TYPE_TEXTAREA)
						$field['options'] = $options;
					if(is_null($postid))
						$field['value'] = ilya_opt(ilya_eqf::FIELD_DEFAULT.$key);
					else
						$field['value'] = ilya_db_single_select(ilya_db_post_meta_selectspec($postid, 'ilya_q_'.$name));
					if ($field['type'] != ilya_eqf::FIELD_TYPE_TEXT && $field['type'] != ilya_eqf::FIELD_TYPE_TEXTAREA && is_array($field['options'])) {
						if($field['type'] == ilya_eqf::FIELD_TYPE_CHECK) {
							if($field['value'] == 0)
								$field['value'] = '';
						} else
							$field['value'] = @$field['options'][$field['value']];
					}
					if ($field['type'] == ilya_eqf::FIELD_TYPE_TEXTAREA) {
						if(isset($options[0]))
							$field['rows'] = $options[0];
						if(empty($field['rows']))
							$field['rows'] = ilya_eqf::FIELD_OPTION_ROWS_DFL;
					}
					break;
				}
				$field['note'] = nl2br(ilya_opt(ilya_eqf::FIELD_NOTE.$key));
				if(isset($ilya_extra_question_fields[$name]['error']))
					$field['error'] = $ilya_extra_question_fields[$name]['error'];
				$this->ilya_eqf_insert_array($fields, $field, $name, ilya_opt(ilya_eqf::FIELD_FORM_POS.$key));
			}
		}
		if($multipart) {
	...
	function ilya_eqf_insert_array(&$items, $insertitem, $insertkey, $findkey) {
		$newitems = array();
		if($findkey == ilya_eqf::FIELD_FORM_POS_TOP) {
			$newitems[$insertkey] = $insertitem;
			foreach($items as $key => $item)
				$newitems[$key] = $item;
		} elseif($findkey == ilya_eqf::FIELD_FORM_POS_BOTTOM) {
			foreach($items as $key => $item)
				$newitems[$key] = $item;
			$newitems[$insertkey] = $insertitem;
		} else {
			if(!array_key_exists($findkey, $items))
				$findkey = ilya_eqf::FIELD_FORM_POS_DFL;
			foreach($items as $key => $item) {
				$newitems[$key] = $item;
				if($key == $findkey)
					$newitems[$insertkey] = $insertitem;
	...
	function ilya_eqf_options($optionstr) {
	...
	function ilya_eqf_output(&$q_view, $position) {
		$output = '';
		$isoutput = false;
		foreach($this->extradata as $key => $item) {
			if($item['position'] == $position) {
				...
				if ($type == ilya_eqf::FIELD_TYPE_TEXTAREA)
					$value = nl2br($value);
				else if ($type == ilya_eqf::FIELD_TYPE_CHECK)
					if ($value == '')
						$value = 0;
				if ($type != ilya_eqf::FIELD_TYPE_TEXT && $type != ilya_eqf::FIELD_TYPE_TEXTAREA && $type != ilya_eqf::FIELD_TYPE_FILE) {
					$options = $this->ilya_eqf_options(ilya_opt(ilya_eqf::FIELD_OPTION.$key));
					if(is_array($options))
						$value = @$options[$value];
				}
				
				if($value == '' && ilya_opt(ilya_eqf::FIELD_HIDE_BLANK.$key))
					continue;
				...
				case ilya_eqf::FIELD_PAGE_POS_INSIDE:
					...
					break;
				case ilya_eqf::FIELD_PAGE_POS_BELOW:
				...
				$title = ilya_opt(ilya_eqf::FIELD_LABEL.$key);
				if ($type == ilya_eqf::FIELD_TYPE_FILE && $value != '') {
				...
				if(ilya_opt(ilya_eqf::FIELD_PAGE_POS.$key) != ilya_eqf::FIELD_PAGE_POS_INSIDE)
					$this->output($output);
				else {
					if(isset($q_view['content'])) {
						$hook = str_replace('^', $key, ilya_eqf::FIELD_PAGE_POS_HOOK);
	...
	function ilya_eqf_get_extradata($postid) {
		$extradata = array();
		for($key=1; $key<=ilya_eqf::FIELD_COUNT_MAX; $key++) {
			if((bool)ilya_opt(ilya_eqf::FIELD_ACTIVE.$key) && (bool)ilya_opt(ilya_eqf::FIELD_DISPLAY.$key)) {
				$name = ilya_eqf::FIELD_BASE_NAME.$key;
				$value = ilya_db_single_select(ilya_db_post_meta_selectspec($postid, 'ilya_q_'.$name));
				if($value == '' && ilya_opt(ilya_eqf::FIELD_HIDE_BLANK.$key))
					continue;
				$extradata[$key] = array(
					'name'=>$name,
					'type'=>ilya_opt(ilya_eqf::FIELD_TYPE.$key),
					'position'=>ilya_opt(ilya_eqf::FIELD_PAGE_POS.$key),
					'value'=>$value,
				);
			}
		}
	...
	function ilya_eqf_file_exist() {
		$fileexist = false;
		foreach($this->extradata as $key => $item) {
			if ($item['type'] == ilya_eqf::FIELD_TYPE_FILE)
				$fileexist = true;
		}
		return $fileexist;
	}
	function ilya_eqf_clearhook(&$q_view) {
		for($key=1; $key<=ilya_eqf::FIELD_COUNT_MAX; $key++) {
			if(isset($q_view['content'])) {
				$hook = str_replace('^', $key, ilya_eqf::FIELD_PAGE_POS_HOOK);
	...
	function ilya_eqf_get_image_width($content) {
