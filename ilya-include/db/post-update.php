...
function ilya_db_post_set_selchildid($questionid, $selchildid, $lastuserid = null, $lastip = null)
{
	ilya_db_query_sub(
		...
		$questionid, ILYA_UPDATE_SELECTED
	);
	...
	if (isset($selchildid) && isset($lastuserid) && isset($lastip)) {
		ilya_db_query_sub(
			...
			ILYA_UPDATE_SELECTED, $lastuserid, bin2hex(@inet_pton($lastip)), $selchildid
		);
	}
}
...
function ilya_db_post_set_closed($questionid, $closedbyid, $lastuserid = null, $lastip = null)
{
	if (isset($lastuserid) || isset($lastip)) {
		ilya_db_query_sub(
			...
			$closedbyid, ILYA_UPDATE_CLOSED, $lastuserid, bin2hex(@inet_pton($lastip)), $questionid
		);
	} else {
		ilya_db_query_sub(
 ...
 */
function ilya_db_post_set_type($postid, $type, $lastuserid = null, $lastip = null, $updatetype = ILYA_UPDATE_TYPE)
{
	if (isset($lastuserid) || isset($lastip)) {
		ilya_db_query_sub(
...
function ilya_db_post_set_parent($postid, $parentid, $lastuserid = null, $lastip = null)
{
	if (isset($lastuserid) || isset($lastip)) {
		ilya_db_query_sub(
			...
			$parentid, ILYA_UPDATE_PARENT, $lastuserid, bin2hex(@inet_pton($lastip)), $postid
		);
	} else {
		ilya_db_query_sub(
 ...
 * @param $name
 */
function ilya_db_post_set_content($postid, $title, $content, $format, $tagstring, $notify, $lastuserid = null, $lastip = null, $updatetype = ILYA_UPDATE_CONTENT, $name = null)
{
	if (isset($lastuserid) || isset($lastip)) {
...
function ilya_db_post_set_category($postid, $categoryid, $lastuserid = null, $lastip = null)
{
	if (isset($lastuserid) || isset($lastip)) {
		ilya_db_query_sub(
			...
			$categoryid, ILYA_UPDATE_CATEGORY, $lastuserid, bin2hex(@inet_pton($lastip)), $postid
		);
	} else {
		ilya_db_query_sub(
...
{
	if (count($postids)) {
		// requires ILYA_CATEGORY_DEPTH=4
		ilya_db_query_sub(
