...
{
	if (defined('ILYA_MYSQL_USERS_PREFIX')) { // check if one of the prefixes is a prefix itself of the other
		if (stripos(ILYA_MYSQL_USERS_PREFIX, ILYA_MYSQL_TABLE_PREFIX) === 0)
			$prefixes = array(ILYA_MYSQL_TABLE_PREFIX);
		elseif (stripos(ILYA_MYSQL_TABLE_PREFIX, ILYA_MYSQL_USERS_PREFIX) === 0)
			$prefixes = array(ILYA_MYSQL_USERS_PREFIX);
		else
			$prefixes = array(ILYA_MYSQL_TABLE_PREFIX, ILYA_MYSQL_USERS_PREFIX);
...
function ilya_db_get_unapproved_users($count)
{
	$results = ilya_db_read_all_assoc(ilya_db_query_sub(
		...
		ILYA_USER_LEVEL_APPROVED, ILYA_USER_FLAGS_USER_BLOCKED, $count
	));
...
function ilya_db_category_child_depth($categoryid)
{
	...
	$result = ilya_db_read_one_assoc(ilya_db_query_sub(
		'SELECT COUNT(child1.categoryid) AS count1, COUNT(child2.categoryid) AS count2, COUNT(child3.categoryid) AS count3 FROM ^categories AS child1 LEFT JOIN ^categories AS child2 ON child2.parentid=child1.categoryid LEFT JOIN ^categories AS child3 ON child3.parentid=child2.categoryid WHERE child1.parentid=#;', // requires ILYA_CATEGORY_DEPTH=4
		$categoryid
	));

	for ($depth = ILYA_CATEGORY_DEPTH - 1; $depth >= 1; $depth--)
...
function ilya_db_categories_recalc_backpaths($firstcategoryid, $lastcategoryid = null)
{
	...
	ilya_db_query_sub(
		...
		$firstcategoryid, $lastcategoryid // requires ILYA_CATEGORY_DEPTH=4
	);
}
