...
/*
	...
	Description: Builders of selectspec arrays (see ilya-db.php) used to specify database SELECTs
...
function ilya_db_posts_basic_selectspec($voteuserid = null, $full = false, $user = true)
{
	...
	if (isset($voteuserid)) {
		...
		array_push($selectspec['arguments'], $voteuserid, $voteuserid, ILYA_ENTITY_QUESTION);
	}
...
function ilya_db_categoryslugs_sql_args($categoryslugs, &$arguments)
{
	...
	if ($levels > 0 && $levels <= ILYA_CATEGORY_DEPTH) {
		$arguments[] = ilya_db_slugs_to_backpath($categoryslugs);
		return (($levels == ILYA_CATEGORY_DEPTH) ? 'categoryid' : ('catidpath' . $levels)) . '=(SELECT categoryid FROM ^categories WHERE backpath=$ LIMIT 1) AND ';
	}
...
function ilya_db_qs_selectspec($voteuserid, $sort, $start, $categoryslugs = null, $createip = null, $specialtype = false, $full = false, $count = null)
{
	...
	$count = isset($count) ? min($count, ILYA_DB_RETRIEVE_QS_AS) : ILYA_DB_RETRIEVE_QS_AS;
...
function ilya_db_recent_a_qs_selectspec($voteuserid, $start, $categoryslugs = null, $createip = null, $specialtype = false, $fullanswers = false, $count = null)
{
	...
	$selectspec['source'] .=
		...
		(ILYA_FINAL_EXTERNAL_USERS ? "" : " LEFT JOIN ^users AS ausers ON aposts.userid=ausers.userid") .
...
function ilya_db_recent_c_qs_selectspec($voteuserid, $start, $categoryslugs = null, $createip = null, $specialtype = false, $fullcomments = false, $count = null)
{
	...
	$selectspec['source'] .=
		...
		(ILYA_FINAL_EXTERNAL_USERS ? "" : " LEFT JOIN ^users AS cusers ON cposts.userid=cusers.userid") .
...
function ilya_db_recent_edit_qs_selectspec($voteuserid, $start, $categoryslugs = null, $lastip = null, $onlyvisible = true, $fulledited = false, $count = null)
{
	...
	$selectspec['source'] .=
		...
		(ILYA_FINAL_EXTERNAL_USERS ? "" : " LEFT JOIN ^users AS editusers ON editposts.lastuserid=editusers.userid") .
...
function ilya_db_flagged_post_qs_selectspec($voteuserid, $start, $fullflagged = false, $count = null)
{
	...
	$selectspec['source'] .=
		...
		(ILYA_FINAL_EXTERNAL_USERS ? "" : " LEFT JOIN ^users AS flagusers ON flagposts.userid=flagusers.userid") .
...
function ilya_db_related_qs_selectspec($voteuserid, $questionid, $count = null)
{
	...
	array_push($selectspec['arguments'], ILYA_IGNORED_WORDS_FREQ, $questionid, ILYA_IGNORED_WORDS_FREQ, ILYA_IGNORED_WORDS_FREQ,
		$questionid, ILYA_IGNORED_WORDS_FREQ, ILYA_IGNORED_WORDS_FREQ, $questionid, ILYA_IGNORED_WORDS_FREQ, $questionid, $count);
...
function ilya_db_search_posts_selectspec($voteuserid, $titlewords, $contentwords, $tagwords, $handlewords, $handle, $start, $full = false, $count = null)
{
	...
	$selectspec['sortdesc'] = 'score';
	array_push($selectspec['arguments'], ILYA_IGNORED_WORDS_FREQ);
	...
	if (!empty($titlewords)) {
		...
		array_push($selectspec['arguments'], ILYA_IGNORED_WORDS_FREQ, $titlewords, ILYA_IGNORED_WORDS_FREQ);
	}
	...
	if (!empty($contentwords)) {
		...
		array_push($selectspec['arguments'], ILYA_IGNORED_WORDS_FREQ, $contentwords, ILYA_IGNORED_WORDS_FREQ);
	}
	...
	if (!empty($tagwords)) {
		...
		array_push($selectspec['arguments'], ILYA_IGNORED_WORDS_FREQ, $tagwords, ILYA_IGNORED_WORDS_FREQ);
	}
	...
	if (!empty($handlewords)) {
		if (ILYA_FINAL_EXTERNAL_USERS) {
			...
			if (count($userids)) {
				...
				array_push($selectspec['arguments'], ILYA_IGNORED_WORDS_FREQ, $userids);
			}
			...
			array_push($selectspec['arguments'], ILYA_IGNORED_WORDS_FREQ, $handlewords);
		}
	}
	...
	if (strlen($handle)) { // to allow searching for multi-word usernames (only works if search query contains full username and nothing else)
		if (ILYA_FINAL_EXTERNAL_USERS) {
			...
			if (count($userids)) {
				...
				array_push($selectspec['arguments'], ILYA_IGNORED_WORDS_FREQ, reset($userids));
			}
			...
			array_push($selectspec['arguments'], ILYA_IGNORED_WORDS_FREQ, $handle);
		}
	}
...
function ilya_db_category_nav_selectspec($slugsorid, $isid, $ispostid = false, $full = false)
{
	...
	$parentselects = array( // requires ILYA_CATEGORY_DEPTH=4
...
function ilya_db_user_recent_qs_selectspec($voteuserid, $identifier, $count = null, $start = 0)
{
	...
	$selectspec['source'] .= " WHERE ^posts.userid=" . (ILYA_FINAL_EXTERNAL_USERS ? "$" : "(SELECT userid FROM ^users WHERE handle=$ LIMIT 1)") . " AND type='Q' ORDER BY ^posts.created DESC LIMIT #,#";
...
function ilya_db_user_recent_a_qs_selectspec($voteuserid, $identifier, $count = null, $start = 0)
{
	...
	$selectspec['source'] .=
		" JOIN ^posts AS aposts ON ^posts.postid=aposts.parentid" .
		" JOIN (SELECT postid FROM ^posts WHERE " .
		" userid=" . (ILYA_FINAL_EXTERNAL_USERS ? "$" : "(SELECT userid FROM ^users WHERE handle=$ LIMIT 1)") .
...
function ilya_db_user_recent_edit_qs_selectspec($voteuserid, $identifier, $count = null)
{
	...
	$selectspec['source'] .=
		...
		" JOIN ^posts AS editposts ON parentposts.postid=IF(LEFT(editposts.type, 1)='Q', editposts.postid, editposts.parentid)" .
		" JOIN (SELECT postid FROM ^posts WHERE " .
		" lastuserid=" . (ILYA_FINAL_EXTERNAL_USERS ? "$" : "(SELECT userid FROM ^users WHERE handle=$ LIMIT 1)") .
...
{
	$count = isset($count) ? $count : ILYA_DB_RETRIEVE_TAGS;
 ...
 * @return array
 */
function ilya_db_user_points_selectspec($identifier, $isuserid = ILYA_FINAL_EXTERNAL_USERS)
{
	return array(
 ...
 * @return array
 */
function ilya_db_user_rank_selectspec($identifier, $isuserid = ILYA_FINAL_EXTERNAL_USERS)
{
	return array(
...
{
	$count = isset($count) ? min($count, ILYA_DB_RETRIEVE_USERS) : ILYA_DB_RETRIEVE_USERS;
...
function ilya_db_users_with_flag_selectspec($flag, $start = 0, $limit = null)
{
	...
	if (isset($limit)) {
		$limit = min($limit, ILYA_DB_RETRIEVE_USERS);
		$source .= ' LIMIT #,#';
...
{
	$count = isset($count) ? min($count, ILYA_DB_RETRIEVE_MESSAGES) : ILYA_DB_RETRIEVE_MESSAGES;
...
function ilya_db_messages_inbox_selectspec($type, $toidentifier, $toisuserid, $start = 0, $limit = null)
{
	...
	if (isset($limit)) {
		$limit = min($limit, ILYA_DB_RETRIEVE_MESSAGES);
		$source .= ' LIMIT #,#';
		$arguments[] = $start;
		$arguments[] = $limit;
	}
...
function ilya_db_user_favorite_qs_selectspec($userid, $limit = null, $start = 0)
{
	...
	$selectspec['arguments'][] = ILYA_ENTITY_QUESTION;

	if (isset($limit)) {
		$limit = min($limit, ILYA_DB_RETRIEVE_QS_AS);
...
function ilya_db_user_favorite_users_selectspec($userid, $limit = null, $start = 0)
{
	...
	$arguments = array($userid, ILYA_ENTITY_USER);
...
function ilya_db_user_favorite_tags_selectspec($userid, $limit = null, $start = 0)
{
	...
	$arguments = array($userid, ILYA_ENTITY_TAG);

	if (isset($limit)) {
		$limit = min($limit, ILYA_DB_RETRIEVE_TAGS);
		$source .= ' LIMIT #,#';
		$arguments[] = $start;
		$arguments[] = $limit;
	}
...
function ilya_db_user_favorite_categories_selectspec($userid)
{
	...
	return array(
		...
		'arguments' => array($userid, ILYA_ENTITY_CATEGORY),
		'sortasc' => 'title',
	);
}
...
function ilya_db_user_favorite_non_qs_selectspec($userid)
{
	...
	return array(
		...
		'arguments' => array(ILYA_ENTITY_USER, ILYA_ENTITY_TAG, ILYA_ENTITY_CATEGORY, $userid, ILYA_ENTITY_QUESTION),
	);
}
 ...
 * favorited and which no longer post to user streams (see long comment in /ilya-include/db/favorites.php).
 * @param $userid
...
function ilya_db_user_updates_selectspec($userid, $forfavorites = true, $forcontent = true)
{
	...
	$nonesql = ilya_db_argument_to_mysql(ILYA_ENTITY_NONE, true);
	...
	$selectspec['source'] .=
		...
		(ILYA_FINAL_EXTERNAL_USERS ? '' : ' LEFT JOIN ^users AS eventusers ON fullevents.lastuserid=eventusers.userid') .
		' LEFT JOIN ^userpoints AS eventuserpoints ON fullevents.lastuserid=eventuserpoints.userid';
	$selectspec['arguments'][] = ILYA_UPDATE_SELECTED;
 ...
 * @return array
 */
function ilya_db_user_levels_selectspec($identifier, $isuserid = ILYA_FINAL_EXTERNAL_USERS, $full = false)
{
	require_once ILYA_INCLUDE_DIR . 'app/updates.php';
	...
	if ($full) {
		...
		array_unshift($selectspec['arguments'], ILYA_ENTITY_CATEGORY);
	}
