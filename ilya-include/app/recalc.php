...
/*
	...
	^posts (catidpath1, catidpath2, catidpath3): hierarchical path to category ids (requires ILYA_CATEGORY_DEPTH=4)
		...
		case 'doreindexcontent_pagereindex':
			...
			if (count($pages)) {
				...
				foreach ($pages as $pageid => $page) {
					if (!($page['flags'] & ILYA_PAGE_FLAGS_EXTERNAL)) {
			...
			if (count($questionids)) {
				...
				foreach ($questionids as $questionid) {
					...
					foreach ($posts as $postid => $post) {
						...
						if ($followonq) {
							$updatetype = ILYA_UPDATE_FOLLOWS;
						...
						foreach ($comments as $comment) {
							foreach ($keyuserids as $keyuserid => $dummy) {
								if ($keyuserid != $comment['userid'] && $keyuserid != @$posts[$parentid]['userid']) {
									ilya_db_event_create_not_entity($keyuserid, $questionid, $comment['postid'], ILYA_UPDATE_FOLLOWS, $comment['userid'], $comment['created']);
								}
							}
...
function ilya_recalc_transition(&$state, $operation)
{
	...
	$next = (ILYA_FINAL_EXTERNAL_USERS && ($operation == 'dorecalcpoints_recalc')) ? '' : 0;
	$done = 0;
