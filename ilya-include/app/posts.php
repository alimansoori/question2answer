...
{
	ilya_post_set_status($postid, $hidden ? ILYA_POST_STATUS_HIDDEN : ILYA_POST_STATUS_NORMAL, $byuserid);
}


/**
 * Change the status of $postid to $status, which should be one of the ILYA_POST_STATUS_* constants defined in
...
function ilya_post_delete($postid)
{
	...
	if (!$oldpost['hidden']) {
		ilya_post_set_status($postid, ILYA_POST_STATUS_HIDDEN, null);
		$oldpost = ilya_post_get_full($postid, 'QAC');
	}
