...
function ilya_admin_check_privileges(&$ilya_content)
{
	...
	} elseif (ilya_get_logged_in_level() < ILYA_USER_LEVEL_ADMIN) {
		$ilya_content = ilya_content_prepare();
...
function ilya_admin_language_options()
{
	...
	foreach (glob(ILYA_LANG_DIR . '*', GLOB_ONLYDIR) as $directory) {
...
function ilya_admin_theme_options()
{
	...
	$metadataUtil = new ILYA_Util_Metadata();
	foreach (glob(ILYA_THEME_DIR . '*', GLOB_ONLYDIR) as $directory) {
...
function ilya_admin_permit_options($widest, $narrowest, $doconfirms = true, $dopoints = true)
{
	...
	$options = array(
		ILYA_PERMIT_ALL => ilya_lang_html('options/permit_all'),
		ILYA_PERMIT_USERS => ilya_lang_html('options/permit_users'),
		ILYA_PERMIT_CONFIRMED => ilya_lang_html('options/permit_confirmed'),
		ILYA_PERMIT_POINTS => ilya_lang_html('options/permit_points'),
		ILYA_PERMIT_POINTS_CONFIRMED => ilya_lang_html('options/permit_points_confirmed'),
		ILYA_PERMIT_APPROVED => ilya_lang_html('options/permit_approved'),
		ILYA_PERMIT_APPROVED_POINTS => ilya_lang_html('options/permit_approved_points'),
		ILYA_PERMIT_EXPERTS => ilya_lang_html('options/permit_experts'),
		ILYA_PERMIT_EDITORS => ilya_lang_html('options/permit_editors'),
		ILYA_PERMIT_MODERATORS => ilya_lang_html('options/permit_moderators'),
		ILYA_PERMIT_ADMINS => ilya_lang_html('options/permit_admins'),
		ILYA_PERMIT_SUPERS => ilya_lang_html('options/permit_supers'),
	);
	...
	if (!$doconfirms) {
		unset($options[ILYA_PERMIT_CONFIRMED]);
		unset($options[ILYA_PERMIT_POINTS_CONFIRMED]);
	}

	if (!$dopoints) {
		unset($options[ILYA_PERMIT_POINTS]);
		unset($options[ILYA_PERMIT_POINTS_CONFIRMED]);
		unset($options[ILYA_PERMIT_APPROVED_POINTS]);
	}

	if (ILYA_FINAL_EXTERNAL_USERS || !ilya_opt('moderate_users')) {
		unset($options[ILYA_PERMIT_APPROVED]);
		unset($options[ILYA_PERMIT_APPROVED_POINTS]);
	}
...
function ilya_admin_sub_navigation()
{
	...
	if (!ILYA_FINAL_EXTERNAL_USERS && ilya_opt('moderate_users') && $level >= ILYA_USER_LEVEL_MODERATOR) {
...
{
	if (file_exists(ILYA_INCLUDE_DIR . 'db/install.php')) // file can be removed for extra security
		include_once ILYA_INCLUDE_DIR . 'db/install.php';

	if (defined('ILYA_DB_VERSION_CURRENT') && ilya_opt('db_version') < ILYA_DB_VERSION_CURRENT && ilya_get_logged_in_level() >= ILYA_USER_LEVEL_ADMIN) {
		return strtr(
	...
	} elseif (defined('ILYA_BLOBS_DIRECTORY') && !is_writable(ILYA_BLOBS_DIRECTORY)) {
		return ilya_lang_html_sub('admin/blobs_directory_error', ilya_html(ILYA_BLOBS_DIRECTORY));
	}
...
function ilya_admin_single_click($entityid, $action)
{
	...
	if (!ILYA_FINAL_EXTERNAL_USERS && ($action == 'userapprove' || $action == 'userblock')) { // approve/block moderated users
		...
		if (isset($useraccount) && ilya_get_logged_in_level() >= ILYA_USER_LEVEL_MODERATOR) {
			switch ($action) {
				case 'userapprove':
					if ($useraccount['level'] <= ILYA_USER_LEVEL_APPROVED) { // don't demote higher level users
						require_once ILYA_INCLUDE_DIR . 'app/users-edit.php';
						ilya_set_user_level($useraccount['userid'], $useraccount['handle'], ILYA_USER_LEVEL_APPROVED, $useraccount['level']);
						return true;
					}
					break;
			...
			switch ($action) {
				case 'approve':
					if ($queued && !ilya_user_post_permit_error('permit_moderate', $post)) {
						ilya_post_set_status($entityid, ILYA_POST_STATUS_NORMAL, $userid);
						return true;
					}
					break;
				...
				case 'reject':
					if ($queued && !ilya_user_post_permit_error('permit_moderate', $post)) {
						ilya_post_set_status($entityid, ILYA_POST_STATUS_HIDDEN, $userid);
						return true;
					}
					break;
