...
{
	ilya_db_event_create_for_entity(ILYA_ENTITY_QUESTION, $questionid, $questionid, $lastpostid, $updatetype, $lastuserid, $timestamp); // anyone who favorited the question

	if (isset($lastuserid) && !ILYA_FINAL_EXTERNAL_USERS)
		ilya_db_event_create_for_entity(ILYA_ENTITY_USER, $lastuserid, $questionid, $lastpostid, $updatetype, $lastuserid, $timestamp); // anyone who favorited the user who did it
...
function ilya_create_event_for_tags($tagstring, $questionid, $updatetype, $lastuserid, $timestamp = null)
{
	...
	foreach ($tagwordids as $wordid) {
		ilya_db_event_create_for_entity(ILYA_ENTITY_TAG, $wordid, $questionid, $questionid, $updatetype, $lastuserid, $timestamp);
	}
}
...
function ilya_create_event_for_category($categoryid, $questionid, $updatetype, $lastuserid, $timestamp = null)
{
	if (isset($categoryid)) {
		...
		foreach ($categories as $category) {
			ilya_db_event_create_for_entity(ILYA_ENTITY_CATEGORY, $category['categoryid'], $questionid, $questionid, $updatetype, $lastuserid, $timestamp);
		}
	}
}
