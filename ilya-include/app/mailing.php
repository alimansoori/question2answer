...
function ilya_mailing_start()
{
	...
	if (strlen(ilya_opt('mailing_last_userid')) == 0) {
		...
		ilya_opt('mailing_last_userid', '0');
		ilya_opt('mailing_total_users', ilya_db_count_users());
		ilya_opt('mailing_done_users', 0);
	}
}
 ...
 */
function ilya_mailing_stop()
{
	ilya_opt('mailing_last_timestamp', '');
	ilya_opt('mailing_last_userid', '');
	ilya_opt('mailing_done_users', '');
	ilya_opt('mailing_total_users', '');
}
...
function ilya_mailing_perform_step()
{
	...
	$thistime = time();
	$lasttime = ilya_opt('mailing_last_timestamp');
	$perminute = ilya_opt('mailing_per_minute');
	...
	ilya_opt('mailing_last_timestamp', $thistime + 30);
	// prevents a parallel call to ilya_mailing_perform_step() from sending messages, unless we're very unlucky with timing (poor man's mutex)

	$sentusers = 0;
	$users = ilya_db_users_get_mailing_next($lastuserid, $count);
	...
	if (count($users)) {
		...
		ilya_opt('mailing_last_userid', $lastuserid);
		ilya_opt('mailing_done_users', ilya_opt('mailing_done_users') + count($users));
		...
		foreach ($users as $user) {
			...
			ilya_mailing_send_one($user['userid'], $user['handle'], $user['email'], $user['emailcode']);
			$sentusers++;
		}

		ilya_opt('mailing_last_timestamp', $lasttime + $sentusers * 60 / $perminute); // can be floating point result, based on number of mails actually sent
...
function ilya_mailing_send_one($userid, $handle, $email, $emailcode)
{
	...
	$unsubscribeurl = ilya_path_absolute('unsubscribe', array('c' => $emailcode, 'u' => $handle));

	return ilya_send_email(array(
		'fromemail' => ilya_opt('mailing_from_email'),
		'fromname' => ilya_opt('mailing_from_name'),
		'toemail' => $email,
		'toname' => $handle,
		'subject' => ilya_opt('mailing_subject'),
		'body' => trim(ilya_opt('mailing_body')) . "\n\n\n" . ilya_lang('users/unsubscribe') . ' ' . $unsubscribeurl,
		'html' => false,
	));
}
...
function ilya_mailing_progress_message()
{
	...
	if (strlen(ilya_opt('mailing_last_userid'))) {
		return strtr(ilya_lang('admin/mailing_progress'), array(
			'^1' => ilya_format_number(ilya_opt('mailing_done_users')),
			'^2' => ilya_format_number(ilya_opt('mailing_total_users')),
		));
	}
