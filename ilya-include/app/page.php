...
{
	if (!ILYA_FINAL_EXTERNAL_USERS && !ilya_is_logged_in()) {
				...
				if (isset($noticeid)) {
					if (!ilya_check_form_security_code('notice-' . $noticeid, ilya_post_text('code')))
						...
						elseif ($noticeid == 'welcome') {
							...
							ilya_db_user_set_flag(ilya_get_logged_in_userid(), ILYA_USER_FLAGS_WELCOME_NOTICE, false);
...
function ilya_get_request_content()
{
	...
	if (isset($routing[$requestlower])) {
		...
		$ilya_content = require ILYA_INCLUDE_DIR . $routing[$requestlower];

	} elseif (isset($routing[$firstlower . '/'])) {
		ilya_set_template($firstlower);
		$ilya_content = require ILYA_INCLUDE_DIR . $routing[$firstlower . '/'];

	} elseif (is_numeric($requestparts[0])) {
		ilya_set_template('question');
		$ilya_content = require ILYA_INCLUDE_DIR . 'pages/question.php';

	} else {
		ilya_set_template(strlen($firstlower) ? $firstlower : 'ilya'); // will be changed later
		$ilya_content = require ILYA_INCLUDE_DIR . 'pages/default.php'; // handles many other pages, including custom pages and page modules
	}

	if ($firstlower == 'admin') {
		$_COOKIE['ilya_admin_last'] = $requestlower; // for navigation tab now...
		setcookie('ilya_admin_last', $_COOKIE['ilya_admin_last'], 0, '/', ILYA_COOKIE_DOMAIN, (bool)ini_get('session.cookie_secure'), true); // ...and in future
	}
...
function ilya_output_content($ilya_content)
{
	...
	if (isset($userid) && $requestlower != 'confirm' && $requestlower != 'account') {
		...
		if (($flags & ILYA_USER_FLAGS_MUST_CONFIRM) && !($flags & ILYA_USER_FLAGS_EMAIL_CONFIRMED) && ilya_opt('confirm_user_emails')) {
			$ilya_content = ilya_content_prepare();
...
function ilya_content_prepare($voting = false, $categoryids = array())
{
	...
	if (ilya_get_logged_in_level() >= ILYA_USER_LEVEL_ADMIN || !ilya_user_maximum_permit_error('permit_moderate') ||
	...
	if (ilya_is_logged_in()) {
		$ilya_content['loggedin'] = ilya_lang_html_sub_split('main/logged_in_x', ILYA_FINAL_EXTERNAL_USERS
	...
	if (ILYA_FINAL_EXTERNAL_USERS || !ilya_is_logged_in()) {
		...
		setcookie('ilya_noticed', 1, time() + 86400 * 3650, '/', ILYA_COOKIE_DOMAIN, (bool)ini_get('session.cookie_secure'), true); // don't show first-time notice if a user has logged in

		if (ilya_opt('show_notice_welcome') && (ilya_get_logged_in_flags() & ILYA_USER_FLAGS_WELCOME_NOTICE)) {
	...
	$ilya_content['script_rel'][] = 'ilya-content/ilya-global.js?' . ILYA_VERSION;
...
{
	return min(max(0, (int)ilya_get('start')), ILYA_MAX_LIMIT_START);
}
