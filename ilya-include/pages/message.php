...
if ($toaccount['flags'] & ILYA_USER_FLAGS_NO_MESSAGES) {
...
switch (ilya_user_permit_error(null, ILYA_LIMIT_MESSAGES)) {
	case 'limit':
...
if (ilya_post_text('domessage')) {
	...
	if (!ilya_check_form_security_code('message-' . $handle, ilya_post_text('code')))
		...
		if (empty($errors)) {
			...
			$canreply = !(ilya_get_logged_in_flags() & ILYA_USER_FLAGS_NO_MESSAGES);
...
if (ilya_opt('show_message_history')) {
	...
	$showmessages = array_slice(array_reverse($recent, true), 0, ILYA_DB_RETRIEVE_MESSAGES);
