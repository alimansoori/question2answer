...
if ($question['answerbutton']) {
	...
	if (ilya_clicked('a_doadd') || $pagestate == 'answer') {
		switch (ilya_user_post_permit_error('permit_post_a', $question, ILYA_LIMIT_ANSWERS)) {
			case 'login':
...
if ($question['editbutton'] || $question['retagcatbutton']) {
	...
	if ($formtype == 'q_edit') { // get tags for auto-completion
		if (ilya_opt('do_complete_tags'))
			$completetags = array_keys(ilya_db_select_with_pending(ilya_db_popular_tags_selectspec(0, ILYA_DB_RETRIEVE_COMPLETE_TAGS)));
		else
			$completetags = array();
	}
}
...
function ilya_page_q_close_q_submit($question, $closepost, &$in, &$errors)
{
	...
	if (!ilya_check_form_security_code('close-' . $question['postid'], ilya_post_text('code'))) {
		...
		// a) URLs could have a fixed numerical path, e.g. http://ilya.mysite.com/1/478/...
		// b) There could be a question title which is just a number, e.g. http://ilya.mysite.com/478/12345/...
	...
	if (!ilya_check_form_security_code('edit-' . $answerid, ilya_post_text($prefix . 'code')))
		...
		if (empty($errors)) {
			...
			if ($in['dotoc'] && (
				...
				if (ilya_user_limits_remaining(ILYA_LIMIT_COMMENTS)) { // already checked 'permit_post_c'
...
function ilya_page_q_do_comment($question, $parent, $commentsfollows, $pagestart, $usecaptcha, &$cnewin, &$cnewerrors, &$formtype, &$formpostid, &$error)
{
	...
	switch (ilya_user_post_permit_error('permit_post_c', $parent, ILYA_LIMIT_COMMENTS)) {
		case 'login':
