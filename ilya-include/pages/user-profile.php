...
list($useraccount, $userprofile, $userfields, $usermessages, $userpoints, $userlevels, $navcategories, $userrank) =
	ilya_db_select_with_pending(
		...
		ILYA_FINAL_EXTERNAL_USERS ? null : ilya_db_user_profile_selectspec($handle, false),
		ILYA_FINAL_EXTERNAL_USERS ? null : ilya_db_userfields_selectspec(),
		ILYA_FINAL_EXTERNAL_USERS ? null : ilya_db_recent_messages_selectspec(null, null, $handle, false, ilya_opt_if_loaded('page_size_wall')),
		ilya_db_user_points_selectspec($identifier),
		ilya_db_user_levels_selectspec($identifier, ILYA_FINAL_EXTERNAL_USERS, true),
		ilya_db_category_nav_selectspec(null, true),
		ilya_db_user_rank_selectspec($identifier)
	);

if (!ILYA_FINAL_EXTERNAL_USERS && $handle !== ilya_get_logged_in_handle()) {
...
if (!ILYA_FINAL_EXTERNAL_USERS) { // if we're using integrated user management, we can know and show more
	...
	if (isset($loginuserid) && $loginuserid != $userid &&
		($loginlevel >= ILYA_USER_LEVEL_SUPER || $loginlevel > $maxuserlevel) &&
		!ilya_user_permit_error()
	) { // can't change self - or someone on your level (or higher, obviously) unless you're a super admin
		if ($loginlevel >= ILYA_USER_LEVEL_SUPER)
			$maxlevelassign = ILYA_USER_LEVEL_SUPER;
		elseif ($loginlevel >= ILYA_USER_LEVEL_ADMIN)
			$maxlevelassign = ILYA_USER_LEVEL_MODERATOR;
		elseif ($loginlevel >= ILYA_USER_LEVEL_MODERATOR)
			$maxlevelassign = ILYA_USER_LEVEL_EXPERT;

		if ($loginlevel >= ILYA_USER_LEVEL_ADMIN)
			$fieldseditable = true;

		if (isset($maxlevelassign) && ($useraccount['flags'] & ILYA_USER_FLAGS_USER_BLOCKED))
			$maxlevelassign = min($maxlevelassign, ILYA_USER_LEVEL_EDITOR); // if blocked, can't promote too high
	}

	$approvebutton = isset($maxlevelassign)
		&& $useraccount['level'] < ILYA_USER_LEVEL_APPROVED
		&& $maxlevelassign >= ILYA_USER_LEVEL_APPROVED
		&& !($useraccount['flags'] & ILYA_USER_FLAGS_USER_BLOCKED)
		...
		if (ilya_clicked('docancel')) {
			...
			if (!ilya_check_form_security_code('user-edit-' . $handle, ilya_post_text('code'))) {
				...
				if (ilya_post_text('removeavatar')) {
					ilya_db_user_set_flag($userid, ILYA_USER_FLAGS_SHOW_AVATAR, false);
					...
					if (ilya_using_categories()) {
						...
						for ($index = 1; $index <= 999; $index++) {
							...
							if (strlen($categoryid) && strlen($inlevel)) {
								$inuserlevels[] = array(
									'entitytype' => ILYA_ENTITY_CATEGORY,
									'entityid' => $categoryid,
	...
	if (ilya_clicked('doapprove') || ilya_clicked('doblock') || ilya_clicked('dounblock') || ilya_clicked('dohideall') || ilya_clicked('dodelete')) {
		if (!ilya_check_form_security_code('user-' . $handle, ilya_post_text('code')))
			...
			if ($approvebutton && ilya_clicked('doapprove')) {
				require_once ILYA_INCLUDE_DIR . 'app/users-edit.php';
				ilya_set_user_level($userid, $useraccount['handle'], ILYA_USER_LEVEL_APPROVED, $useraccount['level']);
				ilya_redirect(ilya_request());
			}

			if (isset($maxlevelassign) && ($maxuserlevel < ILYA_USER_LEVEL_MODERATOR)) {
				...
				if (ilya_clicked('dohideall') && !ilya_user_permit_error('permit_hide_show')) {
					...
					foreach ($postids as $postid)
						ilya_post_set_status($postid, ILYA_POST_STATUS_HIDDEN, $loginuserid);

					ilya_redirect(ilya_request());
				}

				if (ilya_clicked('dodelete') && ($loginlevel >= ILYA_USER_LEVEL_ADMIN)) {
					require_once ILYA_INCLUDE_DIR . 'app/users-edit.php';
...
if ($loginlevel >= ILYA_USER_LEVEL_ADMIN && ilya_clicked('dosetbonus')) {
...
if (isset($loginuserid) && $loginuserid != $useraccount['userid'] && !ILYA_FINAL_EXTERNAL_USERS) {
	$favoritemap = ilya_get_favorite_non_qs_map();
	$favorite = @$favoritemap['user'][$useraccount['userid']];

	$ilya_content['favorite'] = ilya_favorite_form(ILYA_ENTITY_USER, $useraccount['userid'], $favorite,
...
if (!ILYA_FINAL_EXTERNAL_USERS) {
	...
	$ilya_content['form_profile'] = array(
		...
		'fields' => array(
			...
			'level' => array(
				...
				'note' => (($useraccount['flags'] & ILYA_USER_FLAGS_USER_BLOCKED) && isset($maxlevelassign)) ? ilya_lang_html('users/user_blocked') : '',
				'id' => 'level',
			),
		),
	);
	...
	if (ilya_opt('allow_private_messages') && isset($loginuserid) && $loginuserid != $userid && !($useraccount['flags'] & ILYA_USER_FLAGS_NO_MESSAGES) && !$userediting) {
	...
	if ($userediting) {
		if (isset($maxlevelassign)) {
			...
			$showlevels = array(ILYA_USER_LEVEL_BASIC);
			if (ilya_opt('moderate_users'))
				$showlevels[] = ILYA_USER_LEVEL_APPROVED;

			array_push($showlevels, ILYA_USER_LEVEL_EXPERT, ILYA_USER_LEVEL_EDITOR, ILYA_USER_LEVEL_MODERATOR, ILYA_USER_LEVEL_ADMIN, ILYA_USER_LEVEL_SUPER);
			...
			foreach ($showlevels as $showlevel) {
				if ($showlevel <= $maxlevelassign) {
					...
					if ($showlevel > ILYA_USER_LEVEL_BASIC)
			...
			if (ilya_using_categories()) {
				...
				if ($catleveladd || count($userlevels))
					$userlevels[] = array('entitytype' => ILYA_ENTITY_CATEGORY);
		...
		foreach ($userlevels as $userlevel) {
			if ($userlevel['entitytype'] == ILYA_ENTITY_CATEGORY && $userlevel['level'] > $useraccount['level']) {
	...
	foreach ($permitoptions as $permitoption) {
		...
		if (ilya_permit_error($permitoption, $userid, ILYA_USER_LEVEL_APPROVED, ILYA_USER_FLAGS_EMAIL_CONFIRMED, 0) &&
	...
	if ($loginlevel >= ILYA_USER_LEVEL_ADMIN && !ilya_user_permit_error()) {
		$doconfirms = ilya_opt('confirm_user_emails') && $useraccount['level'] < ILYA_USER_LEVEL_EXPERT;
		$isconfirmed = ($useraccount['flags'] & ILYA_USER_FLAGS_EMAIL_CONFIRMED) > 0;
	...
	if ($loginlevel >= ILYA_USER_LEVEL_MODERATOR && !ilya_user_permit_error()) {
	...
	foreach ($userfields as $userfield) {
		if (($userfield['flags'] & ILYA_FIELD_FLAGS_LINK_URL) && !$fieldsediting) {
			...
			$valuehtml = ilya_html($value, (($userfield['flags'] & ILYA_FIELD_FLAGS_MULTI_LINE) && !$fieldsediting));
		}
		...
		if (isset($userfield['permit']) && !$userediting) {
			if ($userfield['permit'] <= ILYA_PERMIT_ADMINS)
				$notehtml = ilya_lang_html('users/only_shown_admins');
			elseif ($userfield['permit'] <= ILYA_PERMIT_MODERATORS)
				$notehtml = ilya_lang_html('users/only_shown_moderators');
			elseif ($userfield['permit'] <= ILYA_PERMIT_EDITORS)
				$notehtml = ilya_lang_html('users/only_shown_editors');
			elseif ($userfield['permit'] <= ILYA_PERMIT_EXPERTS)
	...
	if ($userediting) {
		if ((ilya_opt('avatar_allow_gravatar') && ($useraccount['flags'] & ILYA_USER_FLAGS_SHOW_GRAVATAR)) ||
			(ilya_opt('avatar_allow_upload') && ($useraccount['flags'] & ILYA_USER_FLAGS_SHOW_AVATAR) && isset($useraccount['avatarblobid']))
		) {
		...
		if (isset($maxlevelassign) && $useraccount['level'] < ILYA_USER_LEVEL_MODERATOR) {
			if ($useraccount['flags'] & ILYA_USER_FLAGS_USER_BLOCKED) {
