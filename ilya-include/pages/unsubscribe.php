...
$unsubscribed = (bool) (ilya_get_logged_in_flags() & ILYA_USER_FLAGS_NO_MAILINGS);
...
if (ilya_clicked('dounsubscribe')) {
	if (!ilya_check_form_security_code('unsubscribe', ilya_post_text('formcode'))) {
		...
		if ($isLoggedIn) {
			...
			ilya_db_user_set_flag($loggedInUserId, ILYA_USER_FLAGS_NO_MAILINGS, true);
			$unsubscribed = true;
			...
			if (!empty($inhandle)) {
				...
				if (strtolower(trim(@$userinfo['emailcode'])) == strtolower($incode)) {
					ilya_db_user_set_flag($userinfo['userid'], ILYA_USER_FLAGS_NO_MAILINGS, true);
					$unsubscribed = true;
				}
			}
