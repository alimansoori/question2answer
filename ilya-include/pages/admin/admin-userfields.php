...
elseif (ilya_clicked('dosavefield')) {
	...
	if (!ilya_check_form_security_code('admin/userfields', ilya_post_text('code')))
		...
		if (ilya_post_text('dodelete')) {
			...
			$inflags = $intype | ($inonregister ? ILYA_FIELD_FLAGS_ON_REGISTER : 0);
			...
			if (ilya_strlen($inname) > ILYA_DB_MAX_PROFILE_TITLE_LENGTH)
				$errors['name'] = ilya_lang_sub('main/max_length_x', ILYA_DB_MAX_PROFILE_TITLE_LENGTH);
			...
			if (isset($editfield['fieldid'])) { // changing existing user field
				...
				for ($attempt = 0; $attempt < 1000; $attempt++) {
					...
					$newtag = ilya_substr(implode('-', ilya_string_to_words($inname)), 0, ILYA_DB_MAX_PROFILE_TITLE_LENGTH - strlen($suffix)) . $suffix;
					$uniquetag = true;
...
$typeoptions = array(
	...
	ILYA_FIELD_FLAGS_MULTI_LINE => ilya_lang_html('admin/field_multi_line'),
	ILYA_FIELD_FLAGS_LINK_URL => ilya_lang_html('admin/field_link_url'),
);
...
$ilya_content['form'] = array(
	...
	'fields' => array(
		...
		'type' => array(
			...
			'type' => 'select',
			'options' => $typeoptions,
			'value' => @$typeoptions[isset($intype) ? $intype : (@$editfield['flags'] & (ILYA_FIELD_FLAGS_MULTI_LINE | ILYA_FIELD_FLAGS_LINK_URL))],
		),
		...
		'onregister' => array(
			...
			'type' => 'checkbox',
			'value' => isset($inonregister) ? $inonregister : (@$editfield['flags'] & ILYA_FIELD_FLAGS_ON_REGISTER),
		),
	),
