...
/*
	...
	File: ilya-include/util/sort.php
...
define('ILYA_ARRAY_WITH_FIRST', null); // collect the elements together in the position of the first one found
define('ILYA_ARRAY_WITH_LAST', 0.6); // collect the elements together in the position of the last one found
define('ILYA_ARRAY_AT_START', 0.1); // place all the elements at the start of the array
define('ILYA_ARRAY_AT_END', 0.9); // place all the elements at the end of the array
 ...
 * end of the array). Any of the ILYA_ARRAY_* values defined above can also be passed in the $beforekey parameter.
...
function ilya_array_reorder(&$array, $keys, $beforekey = null, $reorderrelative = true)
{
	...
	if ($beforekey == ILYA_ARRAY_AT_START)
		$offset = 0;
	...
	foreach ($array as $key => $value) {
		...
		if (isset($keyorder[$key])) {
			...
			if ($beforekey == ILYA_ARRAY_WITH_LAST || ($beforekey === ILYA_ARRAY_WITH_FIRST && !isset($offset)))
				$offset = count($newkeys);
	...
	if (!isset($offset)) // also good for ILYA_ARRAY_AT_END
		$offset = count($newkeys);
