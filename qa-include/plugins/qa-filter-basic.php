...
class ilya_filter_basic
{
	...
	{
		if (!strlen($email)) {
			return ilya_lang('users/email_required');
		}
		if (!ilya_email_validate($email)) {
			return ilya_lang('users/email_invalid');
		}
		if (ilya_strlen($email) > QA_DB_MAX_EMAIL_LENGTH) {
			return ilya_lang_sub('main/max_length_x', QA_DB_MAX_EMAIL_LENGTH);
		}
	}
	...
	{
		if (!strlen($handle)) {
			return ilya_lang('users/handle_empty');
		}
		...
		if (ilya_strlen($handle) > QA_DB_MAX_HANDLE_LENGTH) {
			return ilya_lang_sub('main/max_length_x', QA_DB_MAX_HANDLE_LENGTH);
		}
		// check for banned usernames (e.g. "anonymous")
		$wordspreg = ilya_block_words_to_preg(ilya_opt('block_bad_usernames'));
		$blocked = ilya_block_words_match_all($handle, $wordspreg);
		if (!empty($blocked)) {
			return ilya_lang('users/handle_blocked');
		}
	}
	...
	public function filter_question(&$question, &$errors, $oldquestion)
	{
		...
		$qmaxlength = max($qminlength, min(ilya_opt('max_len_q_title'), QA_DB_MAX_TITLE_LENGTH));
		...
		if (isset($question['tags'])) {
			...
			$maxtags = ilya_opt('max_num_q_tags');
			$mintags = min(ilya_opt('min_num_q_tags'), $maxtags);

			if ($counttags < $mintags) {
				$errors['tags'] = ilya_lang_sub('question/min_tags_x', $mintags);
				...
				if (ilya_strlen($tagstring) > QA_DB_MAX_TAGS_LENGTH) { // for storage
					$errors['tags'] = ilya_lang_sub('main/max_length_x', QA_DB_MAX_TAGS_LENGTH);
				}
			}
		}
	...
	{
		$length = isset($input) ? ilya_strlen($input) : 0;

		if ($length < $minlength)
			$errors[$field] = ($minlength == 1) ? ilya_lang('main/field_required') : ilya_lang_sub('main/min_length_x', $minlength);
		elseif (isset($maxlength) && ($length > $maxlength))
			$errors[$field] = ilya_lang_sub('main/max_length_x', $maxlength);
	}
	...
	private function validate_field_length(&$errors, &$post, $key, $minlength, $maxlength, $errorKey = null)
	{
		...
		if (array_key_exists($key, $post)) {
			$length = ilya_strlen($post[$key]);

			if ($length < $minlength) {
				$errors[$errorKey] = $minlength == 1 ? ilya_lang('main/field_required') : ilya_lang_sub('main/min_length_x', $minlength);
			} elseif (isset($maxlength) && ($length > $maxlength)) {
				$errors[$errorKey] = ilya_lang_sub('main/max_length_x', $maxlength);
			}
		}
	}
