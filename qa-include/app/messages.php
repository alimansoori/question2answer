...
function ilya_wall_error_html($fromuserid, $touserid, $touserflags)
{
	...
	if (!QA_FINAL_EXTERNAL_USERS && ilya_opt('allow_user_walls')) {
		if (($touserflags & QA_USER_FLAGS_NO_WALL_POSTS) && !(isset($fromuserid) && $fromuserid == $touserid))
			...
			switch (ilya_user_permit_error('permit_post_wall', QA_LIMIT_WALL_POSTS)) {
				case 'limit':
					return ilya_lang_html('profile/post_wall_limit');
					break;

				case 'login':
					return ilya_insert_login_links(ilya_lang_html('profile/post_wall_must_login'), ilya_request());
					break;

				case 'confirm':
					return ilya_insert_login_links(ilya_lang_html('profile/post_wall_must_confirm'), ilya_request());
					break;

				case 'approve':
					return strtr(ilya_lang_html('profile/post_wall_must_be_approved'), array(
 ...
 * @return mixed
 */
function ilya_wall_add_post($userid, $handle, $cookieid, $touserid, $tohandle, $content, $format)
{
	if (ilya_to_override(__FUNCTION__)) { $args=func_get_args(); return ilya_call_override(__FUNCTION__, $args); }
	...
	ilya_report_event('u_wall_post', $userid, $handle, $cookieid, array(
		'userid' => $touserid,
		'handle' => $tohandle,
		'messageid' => $messageid,
		'content' => $content,
		'format' => $format,
		'text' => ilya_viewer_text($content, $format),
	));
...
/**
 * Deletes the wall post described in $message (as obtained via ilya_db_recent_messages_selectspec()). The deletion was performed
...
function ilya_wall_delete_post($userid, $handle, $cookieid, $message)
{
	...
	ilya_report_event('u_wall_delete', $userid, $handle, $cookieid, array(
...
/**
 * Return the list of messages in $usermessages (as obtained via ilya_db_recent_messages_selectspec()) with additional
 ...
 * @return mixed
 */
function ilya_wall_posts_add_rules($usermessages, $start)
{
	if (ilya_to_override(__FUNCTION__)) { $args=func_get_args(); return ilya_call_override(__FUNCTION__, $args); }

	$userid = ilya_get_logged_in_userid();
	// reuse "Hiding or showing any post" and "Deleting hidden posts" permissions
	$userdeleteall = !(ilya_user_permit_error('permit_hide_show') || ilya_user_permit_error('permit_delete_hidden'));
...
/**
 * Returns an element to add to $ilya_content['message_list']['messages'] for $message (as obtained via
 * ilya_db_recent_messages_selectspec() and then ilya_wall_posts_add_rules()).
 * @param $message
 * @return array
 */
function ilya_wall_post_view($message)
{
	...
	$htmlfields = ilya_message_html_fields($message, $options);
	...
	if ($message['deleteable']) {
		$htmlfields['form'] = array(
			...
			'buttons' => array(
				'delete' => array(
					'tags' => 'name="m' . ilya_html($message['messageid']) . '_dodelete" onclick="return ilya_wall_post_click(' . ilya_js($message['messageid']) . ', this);"',
					'label' => ilya_lang_html('question/delete_button'),
					'popup' => ilya_lang_html('profile/delete_wall_post_popup'),
				),
			),
		);
	}
...
/**
 * Returns an element to add to $ilya_content['message_list']['messages'] with a link to view all wall posts
 * @param $handle
 * @param $start
 * @return array
 */
function ilya_wall_view_more_link($handle, $start)
{
	$url = ilya_path_html('user/' . $handle . '/wall', array('start' => $start));
	return array(
		'content' => '<a href="' . $url . '">' . ilya_lang_html('profile/wall_view_more') . '</a>',
	);
}


/**
 * Hides the private message described in $message (as obtained via ilya_db_messages_inbox_selectspec() or ilya_db_messages_outbox_selectspec()).
