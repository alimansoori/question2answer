...
list($useraccount, $userprofile, $userfields, $usermessages, $userpoints, $userlevels, $navcategories, $userrank) =
	ilya_db_select_with_pending(
		...
		QA_FINAL_EXTERNAL_USERS ? null : ilya_db_user_profile_selectspec($handle, false),
		QA_FINAL_EXTERNAL_USERS ? null : ilya_db_userfields_selectspec(),
		QA_FINAL_EXTERNAL_USERS ? null : ilya_db_recent_messages_selectspec(null, null, $handle, false, ilya_opt_if_loaded('page_size_wall')),
		ilya_db_user_points_selectspec($identifier),
		ilya_db_user_levels_selectspec($identifier, QA_FINAL_EXTERNAL_USERS, true),
		ilya_db_category_nav_selectspec(null, true),
		ilya_db_user_rank_selectspec($identifier)
	);

if (!QA_FINAL_EXTERNAL_USERS && $handle !== ilya_get_logged_in_handle()) {
		...
		if (ilya_clicked('docancel')) {
			...
			if (!ilya_check_form_security_code('user-edit-' . $handle, ilya_post_text('code'))) {
				...
				if (ilya_post_text('removeavatar')) {
					ilya_db_user_set_flag($userid, QA_USER_FLAGS_SHOW_AVATAR, false);
	...
	if (ilya_clicked('doapprove') || ilya_clicked('doblock') || ilya_clicked('dounblock') || ilya_clicked('dohideall') || ilya_clicked('dodelete')) {
		if (!ilya_check_form_security_code('user-' . $handle, ilya_post_text('code')))
			...
			if ($approvebutton && ilya_clicked('doapprove')) {
				require_once QA_INCLUDE_DIR . 'app/users-edit.php';
				ilya_set_user_level($userid, $useraccount['handle'], QA_USER_LEVEL_APPROVED, $useraccount['level']);
				ilya_redirect(ilya_request());
			}
			...
			if (isset($maxlevelassign) && ($maxuserlevel < QA_USER_LEVEL_MODERATOR)) {
				...
				if (ilya_clicked('dohideall') && !ilya_user_permit_error('permit_hide_show')) {
					...
					foreach ($postids as $postid)
						ilya_post_set_status($postid, QA_POST_STATUS_HIDDEN, $loginuserid);

					ilya_redirect(ilya_request());
				}

				if (ilya_clicked('dodelete') && ($loginlevel >= QA_USER_LEVEL_ADMIN)) {
					require_once QA_INCLUDE_DIR . 'app/users-edit.php';
...
if ($loginlevel >= QA_USER_LEVEL_ADMIN && ilya_clicked('dosetbonus')) {
...
if (isset($loginuserid) && $loginuserid != $useraccount['userid'] && !QA_FINAL_EXTERNAL_USERS) {
	...
	$ilya_content['favorite'] = ilya_favorite_form(QA_ENTITY_USER, $useraccount['userid'], $favorite,
...
if (!QA_FINAL_EXTERNAL_USERS) {
	...
	$ilya_content['form_profile'] = array(
		...
		'fields' => array(
			...
			'level' => array(
				...
				'note' => (($useraccount['flags'] & QA_USER_FLAGS_USER_BLOCKED) && isset($maxlevelassign)) ? ilya_lang_html('users/user_blocked') : '',
				'id' => 'level',
			),
		),
	);
	...
	if (ilya_opt('allow_private_messages') && isset($loginuserid) && $loginuserid != $userid && !($useraccount['flags'] & QA_USER_FLAGS_NO_MESSAGES) && !$userediting) {
	...
	foreach ($permitoptions as $permitoption) {
		...
		if (ilya_permit_error($permitoption, $userid, QA_USER_LEVEL_APPROVED, QA_USER_FLAGS_EMAIL_CONFIRMED, 0) &&
	...
	if ($loginlevel >= QA_USER_LEVEL_ADMIN && !ilya_user_permit_error()) {
	...
	if ($loginlevel >= QA_USER_LEVEL_MODERATOR && !ilya_user_permit_error()) {
	...
	foreach ($userfields as $userfield) {
		if (($userfield['flags'] & QA_FIELD_FLAGS_LINK_URL) && !$fieldsediting) {
			...
			$valuehtml = ilya_html($value, (($userfield['flags'] & QA_FIELD_FLAGS_MULTI_LINE) && !$fieldsediting));
		}
	...
	if ($userediting) {
		if ((ilya_opt('avatar_allow_gravatar') && ($useraccount['flags'] & QA_USER_FLAGS_SHOW_GRAVATAR)) ||
			(ilya_opt('avatar_allow_upload') && ($useraccount['flags'] & QA_USER_FLAGS_SHOW_AVATAR) && isset($useraccount['avatarblobid']))
		) {
